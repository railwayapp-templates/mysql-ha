            ...
            Ok(false) => {
                if !no_primary_alerted {
                    ...
                    telemetry.send(TelemetryEvent::NoPrimary {
                        backends: vec!["mysql_primary_backend".to_string()],
                    });
...
/// Returns true if at least one server in mysql_primary_backend is UP.
fn check_primary_health(client: &Client) -> Result<bool> {
    ...
    for line in text.lines() {
        ...
        let svname = cols[1];
        let status = cols[17];
        if pxname == "mysql_primary_backend" && svname != "BACKEND" && status == "UP" {
            return Ok(true);
        }
    }
    Ok(false)
