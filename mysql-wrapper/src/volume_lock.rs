//! Volume-lifetime runtime lock: at most one MySQL container touches this
//! volume at a time.
//!
//! A redeploy (or an instance restart during host maintenance) can leave the
//! old and new containers briefly overlapping on the shared volume. InnoDB's
//! own ibdata lock makes the second mysqld ABORT in that state — a crash-loop
//! whose shape depends on which process wins the race, and which protects
//! only the files InnoDB itself locks. The wrapper therefore holds an
//! exclusive `flock` on a file at the data directory root for the whole life
//! of the supervisor process: a booting container waits for the previous
//! holder's supervisor to exit before anything (config rendering, the
//! password pin, mysqld itself) touches the data directory, and fail-stops
//! loudly on timeout so the restart policy retries the boot.
//!
...
//! outlives mysqld by construction (`process_manager::supervise` reaps it
                ...
                if Instant::now() >= deadline {
                    bail!(
                        "previous container did not release the volume within {wait_secs}s; \
                         refusing to start mysqld against a volume another instance may still be using"
                    );
                }
                file = returned;
