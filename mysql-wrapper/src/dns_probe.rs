...
//! waiting on a scaled-away peer (see `crate::gr`). Transient NXDOMAIN
...
pub async fn probe_name_detailed(name: &str, deadline: Duration) -> (NameVerdict, ProbeDetail) {
    ...
    // proof. Guards against a GR_SEEDS entry declared by raw address.
...
mod tests {
    ...
    fn encode_builds_a_wellformed_question() {
        let packet = encode_query(0xBEEF, "mysql-2.railway.internal.").unwrap();
